...
fillBySymbol(bcfrec::BcfFormat & fmt, const Symbol2CountCoverageSet & symbol2CountCoverageSet12, 
    ...
    if (fmtAD > 0 || is_rescued) {
        ...
        for (unsigned int t = 0; t < 2; t++) {
            ...
            const double any_contam_rate = 2.5e-2; // - (5e-3/3.0)*(double)SYMBOL_TO_INDEL_N_UNITS[symbol];
            ...
            int hetREF_likelim =  (int)(10.0/log(10.00) * 3.0 * MIN(logit2(fr_l / refmul * (1.0 + 0.0),     fa_l / altmul), 0.0)); 
            int hetALT_likelim =  (int)(10.0/log(10.00) * 3.0 * MIN(logit2(fa_l / altmul * (1.0 + 0.0),     fr_l / refmul), 0.0));             
            int homref_likelim =  (int)(10.0/log(10.00) * 3.0 * MIN(logit2(fr_l / refmul * any_contam_rate, fa_l / altmul), 0.0)); 
            int homalt_likelim =  (int)(10.0/log(10.00) * 3.0 * MIN(logit2(fa_l / altmul * any_contam_rate, fr_l / refmul), 0.0)); 
            ...
            int homref_likeval = -(int)calc_binom_10log10_likeratio(any_contam_rate * altmul, da_v, dr_v);                     // hom-alt to REF add error phred by contamination
            int homalt_likeval = -(int)calc_binom_10log10_likeratio(any_contam_rate * refmul, dr_v, da_v);                     // hom-ref to ALT add error phred by contamination
