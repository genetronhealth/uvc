...
fillBySymbol(bcfrec::BcfFormat & fmt, const Symbol2CountCoverageSet & symbol2CountCoverageSet12, 
    ...
    if (fmtAD > 0 || is_rescued) {
        ...
        if (fmt.FA > (0.8 - DBL_EPSILON)) {
            ...
            fmt.GQ = (unsigned int)calc_phred10_likeratio(0.2,  fmtAD, fmt.DP - fmtAD); // homo, so assume hetero is the alternative
        } else if (fmt.FA < (0.2 + DBL_EPSILON)) {
            fmt.GT = (is_novar ? "1/1" : "1/0");
            fmt.GQ = (unsigned int)calc_phred10_likeratio(0.2,  fmtAD, fmt.DP - fmtAD); // homo, so assume hetero is the alternative
        } else {
            fmt.GT = (is_novar ? "0/1" : "0/1");
            fmt.GQ = (unsigned int)calc_phred10_likeratio(0.2,  fmtAD, fmt.DP - fmtAD); // hetero, so assume homo is the alternative
        }
    } else {
        fmt.GT = "./.";
        fmt.GQ = 0;
