...
fillBySymbol(bcfrec::BcfFormat & fmt, const Symbol2CountCoverageSet & symbol2CountCoverageSet12, 
    ...
    if (fmtAD > 0 || is_rescued) {
        ...
        for (unsigned int t = 0; t < 2; t++) {
            ...
            int hetREF_likelim = -(int)(10.0/log(10.00)*2.5*2.0 * MAX(log(0.500 * ref_mulfact / fref), 0.0));       // het-ref to ALT mul error phred
            int hetALT_likelim = -(int)(10.0/log(10.00)*2.5*2.0 * MAX(log(0.500 * ref_mulfact / fa  ), 0.0));       // het-alt to REF mul error phred 
            int homref_likelim = -(int)(10.0/log(10.00)*2.5*2.0 * log(MAX(fa   / t2n_contam_rate,  1.0 / fref)));   // hom-alt to REF mul error phred
            int homalt_likelim = -(int)(10.0/log(10.00)*2.5*2.0 * log(MAX(fref / t2n_contam_rate,  1.0 / fa  )));   // hom-ref to ALT mul error phred
            ...
            int homref_likeval = -(int)calc_binom_10log10_likeratio(t2n_contam_rate    , da,   dref );              // hom-alt to REF add error phred by contamination
            int homalt_likeval = -(int)calc_binom_10log10_likeratio(t2n_contam_rate    , dref, da   );              // hom-ref to ALT add error phred by contamination
