...
const std::vector<BcfFormatStruct> FORMAT_VEC = {
    ...
    BcfFormatStruct("__be"      , 1, BCF_SEP,    "Forward&reverse sequenced-strand depth (bSSD), sequenced-strand bias (bSSB), allele root-mean-squre MQ (bMQ1) and (sum of squared MQ) / (sum of MQ) (bMQ2), where MQ is mapping quality, and the same for base quality BQ [duped]"),
    BcfFormatStruct("bSSD"     , 2, BCF_INTEGER, "see above"),
    BcfFormatStruct("bSSB"     , 2, BCF_INTEGER, "see above"),
