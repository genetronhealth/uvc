...
struct PhredMutationTable {
    ...
    PhredMutationTable(unsigned int CG_TA, unsigned int TA_CG, unsigned int CG_AT, unsigned int transversionany, unsigned int idopen, unsigned int idext)
            : transition_CG_TA(CG_TA), transition_TA_CG(TA_CG), transversion_CG_AT(CG_AT), transversion_any(transversionany), indel_open(idopen), indel_ext(idext) 
            {
    }
...
public:
    ...
    updateByAln(
        ...
        // const unsigned int bias_thres_veryhighBQ = (is_assay_amplicon ? paramset.bias_thres_veryhighBQ_pcr : paramset.bias_thres_veryhighBQ);
        ...
        // const unsigned int frag_pos_L = ((aln->core.isize != 0) ? MAX(MIN(aln->core.pos, aln->core.mpos), aln->core.pos - 1200) : aln->core.pos);
        // const unsigned int frag_pos_R = ((aln->core.isize != 0) ? MIN(MIN(aln->core.pos, aln->core.mpos) + abs(aln->core.isize), rend + 1200) : rend);
        ...
        if (aln->core.pos + paramset.primerlen_min < rend) {
            ...
            const int64_t maxADPl = MAX(prepl[SEG_a_DPf], prepl[SEG_a_DPr]);
            const auto & prepr = seg_format_prep_sets.getByPos(rend - 1 - paramset.primerlen_min);
            const int64_t minADPr = MIN(prepr[SEG_a_DPf], prepr[SEG_a_DPr]);
            const int64_t maxADPr = MAX(prepr[SEG_a_DPf], prepr[SEG_a_DPr]);
            // if covered by at least two amplicons of opposite directions, then take max primer length, and vice versa.
        ...
        // unsigned int tail_clip_len = (isrc ? lclip_len : rclip_len);
                    ...
                    if (i2 > 0) {
                       ...
                       incvalue = non_neg_minus(MIN(qfromBQ2, noindel_phredvalue), micro_nogap_penal) + 1;
                        ...
                        if (TIsBiasUpdated) {
                            ...
                            if (47907026 <= rpos && rpos <= 47907066) { LOG(logINFO) << "dist_to_interfering_indel = " << dist_to_interfering_indel << " at zero-based pos " << rpos << " for read " << bam_get_qname(aln); }
...
if ((!is_assay_amplicon) || (ibeg <= rpos && rpos < iend)) {
                ...
                if (is_ins_at_read_end) {
                    ...
                    //unsigned int base4bit = bam_seqi(bseq, qpos);
                    //unsigned int base3bit = seq_nt16_int[base4bit];
                    //AlignmentSymbol isymbol = AlignmentSymbol(base3bit);
                    ...
                    // proton may generate erroneous indels within true indels, so be more leniant for proton.
                    ...
                    unsigned int qfromBQ2 = ((thisdp * 2 <= neardp || (1 == cigar_oplen && 
                                (xm150 >= 7 || ((lclip_len + 5 >= rpos - aln->core.pos) && isrc) || ((rclip_len + 5 >= rend - aln->core.pos) && !isrc)))) 
...
struct Symbol2CountCoverageSet {
    ...
    updateByAlns3UsingBQ(
        ...
        /*
        if (symbol_to_seg_format_depth_sets.getIncluBegPosition() <= 55249071 - 1 && 55249071 <= symbol_to_seg_format_depth_sets.getExcluEndPosition()) {
            const auto & a = symbol_to_seg_format_depth_sets.getRefByPos(55249071 - 1)[BASE_A];
            const auto & c = symbol_to_seg_format_depth_sets.getRefByPos(55249071 - 1)[BASE_C];
            const auto & g = symbol_to_seg_format_depth_sets.getRefByPos(55249071 - 1)[BASE_A];
            const auto & t = symbol_to_seg_format_depth_sets.getRefByPos(55249071 - 1)[BASE_C];
            // LOG(logINFO) << "For T790M >A, got " << a[SEG_aDPff] << "/" << a[SEG_aDPfr] << "/" << a[SEG_aDPrf] << "/" << a[SEG_aDPrr] << " aDPs!";
            //LOG(logINFO) << "For T790M >C, got " << c[SEG_aDPff] << "/" << c[SEG_aDPfr] << "/" << c[SEG_aDPrf] << "/" << c[SEG_aDPrr] << " aDPs!";
            //LOG(logINFO) << "For T790M >G, got " << g[SEG_aDPff] << "/" << g[SEG_aDPfr] << "/" << g[SEG_aDPrf] << "/" << g[SEG_aDPrr] << " aDPs!";
            //LOG(logINFO) << "For T790M >T, got " << t[SEG_aDPff] << "/" << t[SEG_aDPfr] << "/" << t[SEG_aDPrf] << "/" << t[SEG_aDPrr] << " aDPs!";
        }
        */
        for (const auto & alns2pair2dflag : alns3) {
...
BcfFormat_symbol_init(
    ...
    // const auto & symbol_to_VQ_format_tag_sets = symbol2CountCoverageSet12.symbol_to_VQ_format_tag_sets;
    fmt.DP = fmt.CDP0f[0] + fmt.CDP0r[0];
    fmt.AD = vectorsum(fmt.cDP0f, fmt.cDP0r);
...
BcfFormat_symbol_calc_DPv(
    ...
    if (is_amplicon) {
        ...
        if ((fmt.cDP1r[a] * fmt.CDP1f[0] < fmt.cDP1f[a] * fmt.CDP1r[0]) 
                && (MIN(fmt.aLILf[a], fmt.aRILf[a]) / MAX(fmt.aDPff[a] + fmt.aDPfr[a], 1) > paramset.bias_orientation_counter_avg_end_len)
                && (MIN(fmt.ALILr[0], fmt.ARILr[0]) / MAX(fmt.ADPrf[0] + fmt.ADPrr[0], 1) < paramset.bias_orientation_counter_avg_end_len)) { // fw allele frac > rv allele frac
            cROFA0 = cROFA2 = 2.0;
        }
        if ((fmt.cDP1f[a] * fmt.CDP1r[0] < fmt.cDP1r[a] * fmt.CDP1f[0]) 
                && (MIN(fmt.aLILr[a], fmt.aRILr[a]) / MAX(fmt.aDPrr[a] + fmt.aDPrr[a], 1) > paramset.bias_orientation_counter_avg_end_len)
                && (MIN(fmt.ALILf[0], fmt.ARILf[0]) / MAX(fmt.ADPff[0] + fmt.ADPfr[0], 1) < paramset.bias_orientation_counter_avg_end_len)) { // fw allele frac > rv allele frac
            cROFA0 = cROFA2 = 2.0;
        }
        */  
        if (isSymbolSubstitution(symbol)) {
            // const double amp_bias_by_PCR = pow(ADP + 1, 0.1) * 2;
    ...
    */
    ...
    if (isSymbolIns(symbol) || isSymbolDel(symbol)) {
        ...
        if ((MIN(indelstring.size(), 16) * aDPFA * indel_multialleles_coef >= paramset.nobias_pos_indel_lenfrac_thres) || 
                (MAX(rtr1.tracklen, rtr2.tracklen) >= paramset.nobias_pos_indel_STR_track_len
                && is_in_indel_major_reg && !(fmt.APXM[0] > fmt.APXM[1] * 4))) {
            aLPFA += 2.0;
            aRPFA += 2.0;
            aLBFA += 2.0;
            aRBFA += 2.0;
    ...
    const bool is_strand_r_weak = (f.ADPfr[0] + f.ADPrr[0]) * 20 < (f.ADPff[0] + f.ADPrf[0]);
    const bool is_strand_f_weak = (f.ADPff[0] + f.ADPrf[0]) * 20 < (f.ADPfr[0] + f.ADPrr[0]);
    if (is_strand_r_weak) {
        aLIFA += 4.0;
        aSSFA += 4.0;
    }
...
BcfFormat_symbol_calc_qual(
    ...
    double cMmQf = 10/log(10) * log((fmt.cDPMf[a] + fmt.cDPmf[a] + pow(10, paramset.fam_thres_highBQ / 10.0) * prior_weight) 
            / (fmt.cDPmf[a] + prior_weight));
    prior_weight = 1.0 / (fmt.cDPmr[a] + 1.0);
    double cMmQr = 10/log(10) * log((fmt.cDPMr[a] + fmt.cDPmr[a] + pow(10, paramset.fam_thres_highBQ / 10.0) * prior_weight) 
    ...
    */
    ...
    const int bMQdec = (int)(((refsymbol == symbol) && (ADP > aDP * 2))
            ? BETWEEN((int)(fmt.AMQs[0] / MAX(1, ADP)) - (int)(fmt.aMQs[a] / MAX(1, aDP)), 0, 15) : 0); // ad-hoc for MQ that has ref-bias.
    const int syserr_q = MIN(
            (int)non_neg_minus(MAX(fmt.bMQ[a], (refsymbol == symbol ? 0 : paramset.syserr_minMQ)) + MIN(minMQinc, bDP * 3), bMQdec),
            (SEQUENCING_PLATFORM_IONTORRENT != paramset.sequencing_platform && isSymbolSubstitution(AlignmentSymbol(LAST(fmt.VTI))) ? (fmt.aBQQ[a]) : (1000*1000)));
    ...
    double norm_contam_any_mul_frac = paramset.contam_any_mul_frac; // * (isSymbolSubstitution(symbol) ? 1.0 : 0.5); // indels are less homogeneous, thus reducing the effect of contam
    double contamfrac = norm_contam_any_mul_frac + (1.0 - norm_contam_any_mul_frac) * (tpfa > 0 ? tpfa : 0) * paramset.contam_t2n_mul_frac;
    double errfrac = contamfrac; // + (1.0 - contamfrac) * (refsymbol == symbol ? phred2prob(MIN(rtr1.indelphred, rtr2.indelphred)) : 0) * 0.5;
