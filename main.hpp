...
fill_by_symbol(bcfrec::BcfFormat & fmt, 
    ...
    if (fmtAD > 0 || is_rescued) {
        ...
        if (isSymbolIns(symbol) || isSymbolDel(symbol)) {
             ...
             */
            ref_bias = ((totsize_sum * 100UL) / (totsize_cnt * 100UL + 1UL)) * 3 + (repeatunit.size() * (MAX(1UL, repeatnum) - 1)) + 6 * 2 + 5 * 2; // 6=gap_open/match
        }
        ...
        auto ref_mul_contam_frac = MAX((double)MIN(ref_bias, readlen) / (double)(readlen), any_mul_contam_frac);
